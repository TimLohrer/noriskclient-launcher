...
use crate::minecraft::{ForgeApi, ForgePatcher, NeoForgePatcher, MinecraftLaunchParameters, MinecraftLauncher};
...
pub async fn install_minecraft_version(
    ...
    if modloader_enum == ModLoader::Fabric {
    ...
    } else if modloader_enum == ModLoader::NeoForge {
        // Emit NeoForge installation event
        let neoforge_event_id = Uuid::new_v4();
        state
            .emit_event(EventPayload {
                event_id: neoforge_event_id,
                event_type: EventType::InstallingNeoForge, // Assuming this event type exists
                target_id: Some(profile.id),
        ...
        // Initialize services
        let neoforge_api = crate::minecraft::NeoForgeApi::new();
        let neoforge_libraries_download = crate::minecraft::downloads::NeoForgeLibrariesDownload::new(); // Reuse Forge libraries downloader for now
        let neoforge_installer_download = crate::minecraft::downloads::NeoForgeInstallerDownloadService::new();
        ...
        // Download NeoForge libraries (still uses neoforge_version DTO derived from the installer)
        ...
        launch_params = launch_params
            .with_main_class(&neoforge_version.main_class)
            .with_additional_libraries(libraries)
            .with_additional_jvm_args(NeoForgeArguments::get_jvm_arguments(
                &neoforge_version,
                &LAUNCHER_DIRECTORY.meta_dir().join("libraries"),
        ...
        if let Some(neoforge_profile) = profile_json {
            ...
            let forge_patcher = NeoForgePatcher::new(java_path.clone(), version_id);
            // Use determined installer_path
            forge_patcher
