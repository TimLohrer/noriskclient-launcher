...
impl ForgeArguments {
    ...
    pub fn get_jvm_arguments(forge_version: &ForgeVersion, library_directory: &PathBuf, version_name: &str) -> Vec<String> {
        if let Some(args) = &forge_version.arguments {
            ...
            args.jvm.iter().map(|arg| {
                info!("\nOriginal argument: {}", arg);
                let replaced = arg.replace("${library_directory}", &library_dir)
