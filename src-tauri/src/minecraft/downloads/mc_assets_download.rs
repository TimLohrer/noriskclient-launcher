...
impl MinecraftAssetsDownloadService {
    ...
    pub async fn download_assets_with_progress(&self, asset_index: &AssetIndex, profile_id: Uuid) -> Result<()> {
        ...
        self.emit_progress_event(
            &state, 
            profile_id, 
            &format!("Asset index downloaded. Found {} assets to process", asset_index_content.objects.len()),
            0.15,
            None
        ).await?;
        ...
        for (name, asset) in assets {
            ...
            downloads.push(async move {
                ...
                info!("[Assets Download Task {}] Finished download for: {} ({}/{})", 
            ...
            .inspect(|_| {
                ...
                if total > 0 {
                    ...
                    tokio::spawn({
                        ...
                        let message = format!("Downloading Minecraft assets: {}/{}", completed, total);
                        
                        async move {
                            let event_id = Uuid::new_v4();
                            if let Err(e) = state_clone.emit_event(EventPayload {
                                event_id,
    ...
    pub async fn download_assets(&self, asset_index: &AssetIndex) -> Result<()> {
        self.download_assets_with_progress(asset_index, Uuid::nil()).await
    }
